      ...
      <div className="flex justify-between items-center px-5 md:px-10 py-4">
        ...
        <a href="#" aria-label="Numonics + The Infinite — back to top" className="flex items-center gap-3">
          <svg width="52" height="32" viewBox="0 0 78 48" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            ...
            <rect className="sw-bar" x="8"  y="12" width="5" height="24" fill="#39FF14" style={{ animationDelay: "0.15s" }} />
            ...
            <rect className="sw-bar" x="32" y="0"  width="5" height="48" fill="#39FF14" style={{ animationDelay: "0.45s" }} />
            <rect className="sw-bar" x="40" y="6"  width="5" height="36" fill="#FF1480" style={{ animationDelay: "0.20s" }} />
            <rect className="sw-bar" x="48" y="14" width="5" height="20" fill="#8800FF" style={{ animationDelay: "0.55s" }} />
            <rect className="sw-bar" x="56" y="2"  width="5" height="44" fill="#FF4500" style={{ animationDelay: "0.35s" }} />
        ...
        <div className="flex items-center gap-3">
          <a
            href="#bookings"
            className="hidden md:flex items-center font-heading font-black uppercase tracking-widest text-sm text-black bg-[#39FF14] border-4 border-[#FFD700] px-5 py-2 rounded-full hover:scale-105 transition-transform duration-200 shadow-[4px_4px_0_#FF1480]"
          >
            BOOK US
          </a>
          ...
          >
            ...
            />
            <span
              className={`block w-6 h-0.5 bg-[#39FF14] transition-all duration-200 ${menuOpen ? "opacity-0" : ""}`}
            />
            <span
              className={`block w-6 h-0.5 bg-[#FFD700] transition-all duration-200 ${menuOpen ? "-rotate-45 -translate-y-2" : ""}`}
            />
          </button>
        </div>
      </div>
      ...
      {menuOpen && (
        <div className="md:hidden border-t-4 border-[#FF1480] bg-[#09090E] pattern-dots">
          {NAV_LINKS.map(({ href, label }, i) => {
            const colors = ["#FF1480", "#39FF14", "#FFD700"];
            return (
              <a
                key={href}
                href={href}
          ...
          <a
            href="#bookings"
            onClick={() => setMenuOpen(false)}
            className="block mx-6 my-4 text-center font-heading font-black uppercase tracking-widest text-black bg-[#39FF14] border-4 border-[#FFD700] px-5 py-3 rounded-full"
          >
            BOOK US
          </a>
        </div>
